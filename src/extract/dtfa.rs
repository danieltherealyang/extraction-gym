...
impl TransitionTable {
    pub fn new(rules: Vec<SymbolicRule>) -> TransitionTable {
        ...
        for rule in rules {
            ...
            table_state_map
                .entry(current_rule.borrow().output)
                .or_insert(Vec::new())
                .push(current_rule.clone());
        }
...
impl PartialEq for Context {
    fn eq(&self, other: &Self) -> bool {
        ...
        if self.symbol != other.symbol || self.empty_index != other.empty_index {
            return false;
        }
...
impl ContextSet {
    pub fn new(elements: Vec<ContextId>) -> Self {
        let stored_hash: Hash64 = elements
            .iter()
            .fold(Hash64(0), |acc, &num| acc + Self::hash_element(num));
        return ContextSet {
...
impl FinePartition {
    pub fn new(
        coarse_partition: CoarsePartitionPtr,
    ...
    pub fn cut_from_set(&mut self, b: &FineBlockId, set: &Vec<State>) {
        ...
        self.coarse_partition_ptr
            .borrow_mut()
            .alert_fine_block_split(*b, new_block_id);
    }
    ...
    pub fn generate_obs(&mut self, block: &FineBlock) -> ObservationMap {
        ...
        for output_q in block.elements_iter() {
            let rule_ptrs: Vec<_> = self
                .transition_table_ptr
                .borrow()
                .rule_iter_by_output(*output_q)
                .cloned()
                .collect();
            for rule_ptr in rule_ptrs {
                let mut current_context: Context =
                    Context::new(rule_ptr.borrow().symbol, rule_ptr.borrow().input.clone(), 0);

                let element_iter: Vec<(usize, State)> = current_context
                    .elements_iter()
                    .cloned()
                    .enumerate()
                    .collect();
     ...
     * counts.
     */
    pub fn generate_context_setfn(
        &mut self,
        s_id: CoarseBlockId,
        b_map: &ObservationMap,
    ) -> HashMap<State, ContextSet> {
     ...
     */
    pub fn subtract_obsmap_from_coarse_block(
        &mut self,
        block_to_subtract_from_id: CoarseBlockId,
        b_obsmap: &ObservationMap,
    ) {
        let block_to_subtract_from: &mut ObservationMap =
            self.obs_q.get_mut(&block_to_subtract_from_id).unwrap();
        for (state, context_cnt) in b_obsmap.iter() {
            let to_subtract_from_cnts: &mut HashMap<ContextId, u32> =
                block_to_subtract_from.get_mut(state).unwrap();
    ...
    pub fn splitf(&mut self, b: FineBlock) -> ObservationMap {
        ...
        let mut next_partition: HashMap<FineBlockId, HashMap<ContextSet, Vec<State>>> =
            HashMap::new();
    ...
    pub fn splitfn(&mut self, s: CoarseBlockId, b_obsmap: &ObservationMap) {
        ...
        for (state, context_set) in context_map.iter() {
            let fine_id: FineBlockId = self.get_block_id_from_state(*state);
            let current_map: &mut HashMap<ContextSet, Vec<State>> =
                next_partition.entry(fine_id).or_insert(HashMap::new());
...
impl CoarsePartition {
    ...
    pub fn remove_fine_id(&mut self, fine_id: FineBlockId) {
        ...
        match block.size() {
            0 => {
                self.remove_empty_block(id);
            }
            1 => {
                self.compound_blocks.remove(&id);
            }
            _ => {}
        }
    ...
    pub fn choose_smaller_half(&self, b: CoarseBlockId) -> FineBlockId {
        ...
        if fine_partition.get_block_size(first_fine_id)
            < fine_partition.get_block_size(second_fine_id)
        {
...
impl Dtfa {
    ...
    pub fn minimize(&self) -> Vec<Vec<State>> {
        ...
        while p.borrow_mut().num_compound_blocks() != 0 {
             ...
             */
            r.borrow_mut()
                .subtract_obsmap_from_coarse_block(s, &b_obsmap);
            ...
            r.borrow_mut()
                .set_coarse_block_obsmap(new_coarse_id, b_obsmap);
        }
    ...
    SymbolType: Eq + Hash + Clone,
{
    ...
    pub fn get_accepting_states(&self) -> HashSet<State> {
        let result: HashSet<State> = self
            .accepting_states
            .iter()
            .map(|state| self.to_state[state])
            .collect();
        return result;
    }
    ...
    pub fn get_equiv(&self, state_vec: Vec<Vec<State>>) -> Vec<Vec<StateType>> {
        assert!(state_vec
            .iter()
            .flatten()
            .all(|state| self.to_statetype.contains_key(state)));
        let result: Vec<Vec<StateType>> = state_vec
            .iter()
            .map(|inner_vec| {
                inner_vec
                    .iter()
                    .map(|state| self.to_statetype[&state].clone())
                    .collect()
            })
            .collect();
        return result;
...
fn get_dtfa_states(egraph: &EGraph) -> HashSet<ClassId> {
    let result: HashSet<ClassId> = egraph.classes().keys().cloned().collect();

    assert!(!result.is_empty());
    return result;
}

fn get_dtfa_accepting(runner_roots: &Vec<ClassId>) -> HashSet<ClassId> {
    assert!(!runner_roots.is_empty());
    let result: HashSet<ClassId> = runner_roots.iter().cloned().collect();

    assert!(!result.is_empty());
    return result;
}

fn get_dtfa_symbols(egraph: &EGraph) -> HashSet<NodeId> {
    let result: HashSet<NodeId> = egraph.nodes.keys().cloned().collect();

    assert!(!result.is_empty());
    return result;
}

fn add_dtfa_rules_to_mapper(egraph: &EGraph, dtfa_map: &mut DtfaMapper<ClassId, NodeId>) {
    for (class_id, class) in egraph.classes() {
        let output_state = class_id;
        for node_id in class.nodes.iter() {
            let input_state: Vec<ClassId> = egraph
                .nodes
                .get(node_id)
                .unwrap()
                .children
                .iter()
                .cloned()
                .map(|node_id| egraph.nid_to_cid(&node_id).clone())
                .collect();
            dtfa_map.add_rule(node_id.clone(), input_state, output_state.clone());
        }
    }
}

fn merge_equiv_eclasses<EquivClasses>(egraph: &EGraph, equiv_classes: EquivClasses) -> EGraph
where
    ...
    EquivClasses::Item: IntoIterator<Item = ClassId>,
{
    ...
    for (class_idx, equiv_class) in equiv_classes.into_iter().enumerate() {
        for class_id in equiv_class {
            state_to_equiv.insert(class_id, ClassId::from(format!("{}", class_idx)));
        }
    }

    for (node_id, node) in egraph.nodes.iter() {
        let mut equiv_node: Node = node.clone();
        equiv_node.eclass = state_to_equiv[&node.eclass].clone();
        result.add_node(node_id.clone(), equiv_node);
    }

    let new_roots: HashSet<ClassId> = egraph.root_eclasses.iter().map(
        |class_id| state_to_equiv[&class_id].clone(),
    ).collect();

    result.root_eclasses = new_roots.into_iter().collect();

    return result;
}

pub fn minimize(egraph: &EGraph, runner_roots: &Vec<ClassId>) -> EGraph {
    let mut dtfa_map: DtfaMapper<ClassId, NodeId> = DtfaMapper::new(
        get_dtfa_states(egraph),
        get_dtfa_accepting(runner_roots),
        get_dtfa_symbols(egraph),
    );
    add_dtfa_rules_to_mapper(egraph, &mut dtfa_map);
    ...
    return merge_equiv_eclasses(egraph, dtfa_map.get_equiv(dtfa.minimize()));
}
